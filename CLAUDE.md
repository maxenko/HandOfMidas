...
### ETH session shading (legacy chart default)
TradingView-matching pre/post-market band tints ship on the legacy chart by default. `CandleBuffer` carries a per-bar `sessions: Vec<u8>` SoA column, classified host-side at conversion via `HeuristicSymbolResolver` (XNYS calendar for US equities, `CryptoSpotCalendar` for crypto — never RTH on ETH bars). `compute_session_bands` in `midas-chart` walks the visible range, groups Pre/Post runs, and emits filled rectangles trimmed to the data extent. Per-panel `show_extended_hours` + `show_extended_hours_bands` config knobs (default `true`) drive `MarketDataRouter::historical_bars(use_rth)`, which the IB backend translates to `TradingHours::{Regular | Extended}`; the sim backend emits 04:00–20:00 ET bars when `synthetic_includes_eth = true` AND the call asks `use_rth = false`. The new `session_chart` window keeps its own `SessionBandLayer` painting full-calendar-window bands; the two surfaces are colour-aligned (parity test in `desktop/win/tests/eth_shading_palette_parity.rs` runs in the non-blocking `desktop_session_chart_tests` CI job until flip-to-required lands) but paint different *extents* by design until a future unification. See `plan/session-aware-charts/eth-shading.md` for the full design.
