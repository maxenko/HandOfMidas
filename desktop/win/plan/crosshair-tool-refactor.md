...
- **Visibility rule.** Only visible when the left mouse button is held
  ...
  and `DraggingLevel` interactions.
...
- **Level placement preview.** In `PlacingLevel` mode the crosshair's
  horizontal line is repurposed as a preview line. When Alt is not held,
  the Y position snaps to the nearest OHLC value of nearby candles.
...
| PlacingLevel override | `interaction.rs` | Early return in `handle_mouse_moved` when `PlacingLevel` |
| Cancel clears | `interaction.rs` | `handle_key_pressed(Escape)`, `CancelPlacing` action handler in `state.rs` |
| State mutation | `state.rs` | `apply_action(SetCrosshair)` and `apply_action(ClearCrosshair)` |
| Normal-mode compute | `compute.rs` | `compute_crosshair()` (~90 lines): cursor-to-time, snap to candle, build labels + OHLCV |
| Collapsed-mode compute | `compute.rs` | `compute_collapsed_crosshair()` (~90 lines): index-space snap, same label + OHLCV build |
| OHLC snap for levels | `compute.rs` | `snap_crosshair_to_ohlc()` (~80 lines): mutates `CrosshairRender.horizontal_y` |
| Scene output | `scene.rs` | `ChartScene.crosshair: Option<CrosshairRender>` |
| Input contract | `input.rs` | `ChartInput.crosshair: Option<(f32, f32)>` |
| Render data struct | `instances.rs` | `CrosshairRender`, `OhlcvOverlay`, `AxisLabel` |
| Widget snapshot sync | `chart_widget.rs` | `ChartRenderSnapshot.crosshair_pos`, crosshair read from `chart_state` or `snapshot` |
| GPU instance build | `chart_widget.rs` | `crosshair_to_instances()`: two `GridLineInstance` rects |
| GPU upload | `chart_widget.rs` | `prepare()`: `resources.crosshair_lines`, preview line for `PlacingLevel` |
...
4. `PlacingLevel` early return in `handle_mouse_moved` -- overrides
   the normal visibility rule.
...
- `handle_mouse_moved` (PlacingLevel tracking)
...
- `handle_mouse_pressed` (PlacingLevel left-click placement --
...
`PlacingLevel` mode hijacks the crosshair for its preview line:

- `handle_mouse_moved` has a special early return that sets
  `crosshair_pos` directly on `ChartState` and emits `SetCrosshair`
  regardless of `left_mouse_down`.
- `compute_chart_scene` calls `snap_crosshair_to_ohlc()` which mutates
  the already-built `CrosshairRender` to change `horizontal_y`.
- `prepare()` in `chart_widget.rs` checks `self.placing_level` and adds
  extra `GridLineInstance` rects (blue preview line + glow) from the
  crosshair position.
- The snapped price is written back to `ChartWidgetState.snapped_price`
  via a `Cell` and then read on the next click to create the level.
...
- `update()` syncs `crosshair_pos` (clearing it when exiting
  `PlacingLevel`, restoring from snapshot)
...
- `on_mouse_move` updates `cursor_pos` and clears it if `!in_bounds`
  (for `Tracking` mode; `Preview` mode ignores bounds).
...
ChartAction::SetCrosshair { x, y } => {
    self.crosshair.on_mouse_move(x, y, true);
...
That is a Phase 5 cleanup -- for the initial refactor, keeping the
...
```rust
// In handle_mouse_moved, PlacingLevel branch:
if matches!(state.interaction_mode, InteractionMode::PlacingLevel) {
...
// In handle_mouse_pressed, PlacingLevel left-click:
state.crosshair.exit_preview();

// In handle_key_pressed, Escape while PlacingLevel:
state.crosshair.exit_preview();
```

The `enter_preview` / `exit_preview` methods make the intent explicit.
...
### Phase 2: Move crosshair state into CrosshairTool

**Files changed:** `state.rs`, `crosshair_tool.rs`

1. Add `pub crosshair: CrosshairTool` field to `ChartState`.
2. In `ChartState::new()`, initialize with `CrosshairTool::new()`.
3. Keep `crosshair_pos` and `left_mouse_down` as deprecated aliases
   that delegate to the tool:
   ```rust
   // Temporary compatibility -- removed in Phase 5.
   pub fn crosshair_pos(&self) -> Option<(f32, f32)> {
       self.crosshair.render_pos()
   }
   ```
4. Update `apply_action(SetCrosshair)` and `apply_action(ClearCrosshair)`
   to call `CrosshairTool` methods.
5. Update `apply_action(CancelPlacing)` to call `exit_preview()`.

**Tests:** All existing tests pass. The deprecated accessors keep
downstream code working.

### Phase 3: Update interaction.rs

**Files changed:** `interaction.rs`

1. Replace `state.left_mouse_down = true` with
   `state.crosshair.on_left_press(x, y)`.
2. Replace `state.left_mouse_down = false` with
   `state.crosshair.on_left_release()`.
3. Replace `state.crosshair_pos = Some(...)` with
   `state.crosshair.on_mouse_move(x, y, in_bounds)`.
4. Replace `state.crosshair_pos = None` with appropriate tool method.
5. Replace `state.left_mouse_down` reads with
   `state.crosshair.left_mouse_down()`.
6. Replace the `PlacingLevel` early return's direct state manipulation
   with `state.crosshair.enter_preview(x, y)`.
7. Replace escape/cancel placement's `state.crosshair_pos = None` with
   `state.crosshair.exit_preview()`.
8. Remove the `dragging_handle` visibility check from
   `handle_mouse_moved` -- the interaction layer already does not set
   `left_mouse_down` when entering those modes, so the tool's mode
   will be `Hidden` during handle drags.

**Verification:** The `dragging_handle` suppression works because:
- Volume handle press: early return before `state.left_mouse_down = true` (line 528-533). After refactor, `on_left_press` is not called.
- Timeline border press: early return before `state.left_mouse_down = true` (line 536-543). Same.
- Level drag: enters `PendingDrag` first (which does call `on_left_press`), then transitions to `DraggingLevel`. At this point the crosshair is already in `Tracking` mode. The `on_mouse_move` call should suppress the crosshair during `DraggingLevel`. Solution: add a `suppress()` / `unsuppress()` method pair, or have the interaction layer call `force_hide()` when transitioning to `DraggingLevel`.

Refined approach for `DraggingLevel` suppression:

```rust
// In handle_mouse_moved, when PendingDrag transitions to DraggingLevel:
InteractionMode::PendingDrag { .. } => {
    if dist >= DRAG_THRESHOLD_PX {
        if let Some((level_id, grab_offset)) = hit_test_levels(...) {
            state.interaction_mode = InteractionMode::DraggingLevel { ... };
            state.crosshair.force_hide();  // Suppress crosshair during level drag
            // ...
        }
    }
}
...
And on release from `DraggingLevel`, `on_left_release()` keeps it hidden
(which it would anyway, since mode transitions to `Hidden`).

**Tests:** All existing interaction tests pass with updated assertions.

### Phase 4: Consolidate compute functions
...
5. Verify that `snap_crosshair_to_ohlc()` still works correctly with
   the consolidated output.

**Tests:** All existing crosshair compute tests pass. Add a test for
`build_crosshair_data()` directly.

### Phase 5: Simplify widget layer and remove deprecated aliases

**Files changed:** `chart_widget.rs`, `state.rs`
...
7. Consider removing `SetCrosshair`/`ClearCrosshair` from `ChartAction`
   ...
   methods called from `interaction.rs`. This would eliminate the
   "action emitted from many places" problem entirely. The dirty flag
...
### 5.1 PlacingLevel / Preview mode edge cases

**Risk:** The level tool temporarily suspends PlacingLevel for right-click
panning or middle-click scaling. During this suspension, the widget sync
re-enters PlacingLevel when the interaction returns to Idle. The crosshair
tool needs to handle this correctly.

**Mitigation:** The widget sync already calls
`chart_state.interaction_mode = InteractionMode::PlacingLevel` when
re-entering. This should also call `crosshair.enter_preview(x, y)` with
the last known cursor position. Store the last cursor position inside
`CrosshairTool` so it can be restored.
...
5), the dirty flag must still be marked when crosshair visibility or
position changes.
...
**Risk:** The `DraggingLevel` suppression relies on `force_hide()` being
called at the right transition point. Missing it would show a crosshair
during level drags.

**Mitigation:** Add explicit integration tests:
- Start left press (crosshair visible)
- Move past drag threshold near a level (enters DraggingLevel)
- Assert crosshair is not visible
- Release mouse (exits DraggingLevel)
- Assert crosshair is not visible (left released)
...
| `interaction.rs` | Use `CrosshairTool` methods instead of direct field access | 3 |
| `compute.rs` | Extract `build_crosshair_data()`, simplify two compute fns | 4 |
| `chart_widget.rs` | Sync `CrosshairTool`, simplify crosshair reads | 5 |
| `input.rs` | No structural change (reads from `render_pos()`) | 5 |
| `scene.rs` | No change | -- |
...
- Left press near level -> drag: crosshair NOT visible
- PlacingLevel mode -> move: crosshair visible (preview)
- PlacingLevel -> right-click pan -> release -> crosshair restored

### Compute tests (Phase 4)

- `build_crosshair_data()` produces correct labels and OHLCV
- Normal and collapsed modes produce identical OHLCV for same candle
- `snap_crosshair_to_ohlc()` works with consolidated output
...
|---|---|---|
| Phase 1 | ~120 (struct + tests) | +120 |
| Phase 2 | ~30 changed, ~10 new | +10 |
| Phase 3 | ~50 changed | 0 |
| Phase 4 | ~60 new helper, ~120 simplified | -60 |
| Phase 5 | ~40 changed, ~20 removed | -20 |
| **Total** | | **+50** |

The net increase is modest because the compute function consolidation
(Phase 4) and deprecated field removal (Phase 5) offset the new struct.
