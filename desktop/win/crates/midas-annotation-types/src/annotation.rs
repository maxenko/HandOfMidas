//! Core annotation types: `Annotation`, `AnnotationId`, `AnnotationKind`,
//! `Presence`.
//!
//! Moved verbatim from `midas-chart/src/widget/mod.rs` (Slice A1). The
//! shape is preserved exactly so existing on-disk JSON (e.g.
//! `tests/fixtures/annotations_v1_pre_decorator.json`) round-trips
//! through this crate without re-encoding drift.

use crate::levels::HorizontalLevel;
use crate::marker::MarkerAnnotation;
use crate::order_bracket::OrderBracket;
...
///
/// **Wire format lock:** every variant carries an explicit
/// `#[serde(rename = "...")]` matching the current PascalCase tag the
/// on-disk JSON uses (see `tests/fixtures/annotations_v1_pre_decorator.json`).
/// This pin is intentional: it prevents a future Rust-side rename from
/// silently breaking saved annotation files. Any change here is a
/// migration.
